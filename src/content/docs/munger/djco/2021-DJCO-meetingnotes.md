---
title: "2021年Daily Journal股东会问答"
description: "2021年查理·芒格在Daily Journal股东会的问答记录"
template: splash
---
...
fetch('/value-investment-library/external/munger/djco/2021-DJCO-问答.html')
  .then(r => r.text())
  .then(html => {
    ...
    paragraphs.forEach(p => {
      if (p.match(/Q\s*\d|Q:|问题\s*d|提问|提问\d/) && !p.match(/^芒格/)) {
        result += '<p class="question">' + p + '</p>';
      } else if (p.match(/^芒格|芒格：|查理/)) {
...
:root[data-theme='light'] #letter-content h2,
:root[data-theme='light'] #letter-content h3 {
  color: #1a1a1a;
}
</style>